...
> NOTA DE VIGENCIA (2026-09-10, `docs/arquitectura/DECISION_PRONOSTICO_GLOBAL_v1.0.md`): el modelo oficial es UN pronostico por `UserId + MatchId`, reutilizado en todas las ligas compatibles; evaluaciones/rankings por liga. Las menciones en este archivo a identidad `LeagueId+UserId+MatchId` y a "pronosticos nunca se comparten entre ligas" (Sprint 8.5, Fase 1 y Etapas 1/2/2.5) quedan SIN EFECTO. Se conservan como historial sin reescribir.

**Cierre documental posterior a `f2fdac8` (2026-09-11, sin commit):** cierre operativo de lo ya implementado, sin funcionalidad nueva. HEAD funcional de referencia: `f2fdac8` (`prueba-glm-ui`, sincronizada 0/0 con `origin`). Queda cerrado: Base Inicial v1.0 finalizada; Login visual EL NENE / arquitectura Appearance; P0 Demo funcional (`docs/demos/PlayPredict_DEMO_FUNCIONAL_v1.md`); P1.1 gestión de Ligas de Amigos (`docs/arquitectura/POLITICA_MEMBRESIA_LIGAS_v1.0.md`); P1.2 Importar Fixture XLSX (`/admin/fixture/import`, hoja `IMPORTAR_PARTIDOS` con `FECHA_NRO, FECHA, HORA, LOCAL, VISITANTE, ESTADO`); identidad por email `admin@playpredict.local` / `usuario@playpredict.local` con cambio de email desde Perfil (`ProfilePage` + `ChangeEmailForm`); navegación ADMIN / volver al Dashboard. Backend 248/248 tests al cierre de `f2fdac8`; DB0 validada por restore (`docs/database/backups/README.md`). Base Inicial: 2 users, 30 teams, 1045 players, 60 matches, 0 predictions. Plantilla oficial versionada: `docs/plantillas/PlayPredict_Plantilla_Importar_Fixture.xlsx` + `docs/plantillas/README.md` (3 filas ficticias, validada contra `SpreadsheetReader`: 3 matches, 0 issues).
