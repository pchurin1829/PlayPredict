...
export default function MatchResultModal({ match, onClose, onSaved }: MatchResultModalProps) {
  ...
  const [scorers, setScorers] = useState(match.scorers
    ...
    .map(s => ({ teamPlayerId: s.teamPlayerId, goals: s.goals })))
        ...
        <form onSubmit={handleSubmit}>
          <div className="result-scoreboard">
            ...
            <div className="result-scoreboard__score">
              ...
              />
              <strong>−</strong>
              <input id="awayGoals" aria-label={`Goles de ${match.participantAway}`} type="number" min={0} max={99} inputMode="numeric" value={awayGoals} onFocus={selectValue} onClick={selectValue} onChange={(e) => changeScore('away', Number(e.target.value))} />
            </div>
          ...
          {players.length > 0 && <section className="result-scorers"><h3>Goleadores <small>(opcional)</small></h3><p className="admin-help">Si no se informan, Jugador Preferido queda en 0 puntos hasta completar el detalle.</p>
            {scorers.map((row,index) => <div className="result-scorer-row" key={index}><select aria-label={`Goleador ${index + 1}`} value={row.teamPlayerId} onChange={e => setScorers(current => current.map((x,i) => i===index ? {...x,teamPlayerId:Number(e.target.value)} : x))}><option value={0}>Seleccionar jugador</option>{homeGoals > 0 && <optgroup label={match.participantHome}>{eligiblePlayers.filter(p=>p.teamId===match.homeTeamId).map(p=><option value={p.id} key={p.id}>{p.displayName}</option>)}</optgroup>}{awayGoals > 0 && <optgroup label={match.participantAway}>{eligiblePlayers.filter(p=>p.teamId===match.awayTeamId).map(p=><option value={p.id} key={p.id}>{p.displayName}</option>)}</optgroup>}</select><input aria-label="Cantidad de goles" type="number" min={1} max={99} inputMode="numeric" value={row.goals} onFocus={selectValue} onChange={e=>setScorers(current=>current.map((x,i)=>i===index?{...x,goals:Math.max(1,Number(e.target.value))}:x))}/><button type="button" className="btn btn-tertiary" onClick={()=>setScorers(current=>current.filter((_,i)=>i!==index))}>Quitar</button></div>)}
            <button type="button" className="btn btn-secondary" disabled={!canAddScorer} onClick={()=>setScorers(current=>[...current,{teamPlayerId:0,goals:1}])}>+ Agregar goleador</button>
            ...
            {fieldErrors.scorers?.map(message => <span className="form-field-error" key={message}>{message}</span>)}
          </section>}
