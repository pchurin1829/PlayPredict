...
export default function AdminOfficialLeagueFormPage() {
  ...
  const shownExact = useGeneralScoring ? company.generalExactScorePoints : exact
  const shownCorrect = useGeneralScoring ? company.generalCorrectOutcomePoints : correct
  const shownIncorrect = useGeneralScoring ? company.generalIncorrectPoints : incorrect
  const shownPreferred = useGeneralScoring ? company.generalPreferredPlayerEnabled : preferred
  const shownPerGoal = useGeneralScoring ? company.generalPreferredPlayerPointsPerGoal : perGoal
  const shownPositions = useGeneralScoring ? company.generalPreferredPlayerPositions : positions
  ...
  return (
    <div>
      ...
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        {scopeType === 'RoundRange' && <div className="form-row"><div className="form-field"><label htmlFor="officialFrom">Desde fecha</label><select id="officialFrom" value={roundFromId} required onChange={(e) => setRoundFromId(e.target.value ? Number(e.target.value) : '')}><option value="">Seleccionar...</option>{rounds.map((round) => <option key={round.id} value={round.id}>{roundDisplayName(round)}</option>)}</select>{fieldErrors.roundFromId && <span className="form-field-error">{fieldErrors.roundFromId[0]}</span>}</div><div className="form-field"><label htmlFor="officialTo">Hasta fecha</label><select id="officialTo" value={roundToId} required onChange={(e) => setRoundToId(e.target.value ? Number(e.target.value) : '')}><option value="">Seleccionar...</option>{rounds.map((round) => <option key={round.id} value={round.id}>{roundDisplayName(round)}</option>)}</select>{fieldErrors.roundToId && <span className="form-field-error">{fieldErrors.roundToId[0]}</span>}</div></div>}
        ...
        <section className="scoring-special-section">
          <h2>Configuración de juego</h2>
          <label className="form-field form-checkbox"><input type="checkbox" checked={useGeneralScoring} onChange={e=>setUseGeneralScoring(e.target.checked)}/><span>Usar configuración general</span></label>
          {useGeneralScoring && <p className="form-help scoring-inherited-note">Valores heredados de la configuración general vigente de {companyName}. Se actualizarán si cambia la configuración de la empresa.</p>}
          <fieldset disabled={useGeneralScoring} className={useGeneralScoring ? 'scoring-fields scoring-fields--inherited' : 'scoring-fields'}>
            <legend>{useGeneralScoring ? 'Configuración heredada' : 'Configuración propia'}</legend>
            <div className="form-row"><label className="form-field">Marcador exacto<input type="number" min="0" value={shownExact} onChange={e=>setExact(Math.max(0,Number(e.target.value)))}/></label><label className="form-field">Resultado correcto<input type="number" min="0" value={shownCorrect} onChange={e=>setCorrect(Math.max(0,Number(e.target.value)))}/></label><label className="form-field">Incorrecto<input type="number" min="0" value={shownIncorrect} onChange={e=>setIncorrect(Math.max(0,Number(e.target.value)))}/></label></div>
            <label className="form-field form-checkbox"><input type="checkbox" checked={shownPreferred} onChange={e=>setPreferred(e.target.checked)}/><span>Jugador Preferido habilitado</span></label>
            <label className="form-field">Puntos por gol<input type="number" min="0" disabled={useGeneralScoring || !shownPreferred} value={shownPerGoal} onChange={e=>setPerGoal(Math.max(0,Number(e.target.value)))}/></label>
            <div className="scoring-position-options">{PLAYER_POSITIONS.map(position=><label className="form-checkbox scoring-position-option" key={position}><input type="checkbox" disabled={useGeneralScoring || !shownPreferred} checked={shownPositions.includes(position)} onChange={e=>setPositions(current=>e.target.checked?[...current,position]:current.filter(x=>x!==position))}/><span>{position}</span></label>)}</div>
          </fieldset>
        </section>
