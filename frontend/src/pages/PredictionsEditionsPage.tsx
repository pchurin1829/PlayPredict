...
export default function PredictionsEditionsPage() {
  ...
  return (
    <div>
      <div className="breadcrumb">
        <Link to="/predictions">← Competencias</Link>
      </div>
      <div className="admin-header">
        <h1>Pronósticos — Ediciones {competition ? `— ${competition.name}` : ''}</h1>
      </div>
            ...
            <tbody>
              {editions.map((ed) => (
                <tr
                  ...
                  onClick={() => navigate(`/predictions/editions/${ed.id}/rounds`)}
                >
                  <td>{ed.name}</td>
