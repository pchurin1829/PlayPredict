...
export default function PredictionsRoundsPage() {
  ...
  return (
    <div>
      <div className="breadcrumb">
        {edition && (
          <Link to={`/predictions/competitions/${edition.competitionId}/editions`}>← Ediciones</Link>
        )}
      </div>
      <div className="admin-header">
        <h1>Pronósticos — Fechas {edition ? `— ${edition.name}` : ''}</h1>
      </div>
            ...
            <tbody>
              {rounds.map((r) => (
                <tr
                  ...
                  onClick={() => navigate(`/predictions/rounds/${r.id}`)}
                >
                  <td>{r.order}</td>
                  <td>{r.name}</td>
                </tr>
